...
#define MAX_FILE_NAME_SIZE 256 // WAL file names are likely to be no more than 62 bytes, but extra for safety
...
#define RDMA_CLIENT_ADDR "192.168.252.212:8888"
...
#define LOG_PATH "/hdd2/rdma-libs/"
#define LOG_PATH_SIZE 16

/* CONSTANTS to define control message type */
typedef enum {
    RDMA_INIT_OP,
    RDMA_OPEN_FILE,
    RDMA_LSEEK_FILE,
...
typedef struct rdmaFlushResponse {
    uint64_t lsn_offset;
    uint32_t lsn_partition;
    int error; // store the errno in case of a failure
...
typedef struct rdmaReadFileResponse {
    ...
    bool end; // denotes end of all chunks
